...
打开终端打印的前端地址。服务首次启动会创建两把不同的随机密钥，并且**只打印密钥文件路径，不打印密钥值**。打开该本地文件，将 `admin_token` 的值粘贴到界面的“本机管理密钥”；将 `collector_token` 的值粘贴到扩展设置。前端密钥仅保存在当前页面内存，刷新后需要重新连接。不要把密钥写进 URL、前端环境变量、截图或 Git。
...
连接管理界面后可以下载页面记录备份、删除单条/全部记录、从备份替换恢复。删除或恢复前先暂停扩展并清空待同步队列，避免旧内容重新上传；浏览新页面后仍可再次采集。删除会同时清空分析缓存，不能删除已经下载到其他位置的备份，也不保证磁盘取证级擦除。
